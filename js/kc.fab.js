        ...
        base.init = function(){
            ...
            if (base.links.length > 0){
                ...
                for (var i = 0; i < base.links.length; i++) {
                    ...
                    //get element ID if exists
                    onC = "";
                    if( typeof(base.links[i].dataTarget) != "undefined" ){
                        onC = "data-target='"+base.links[i].dataTarget+"'";
                    }

                    //modal propreties
                    sub_fab_btns_dom += "<div><button data-toggle='modal' "+id_elem+" "+onC+" class='sub_fab_btn' data-link-title='"+base.links[i].title+"' style='"+bg_color_style+"'><span style='"+color_style+"'>"+base.links[i].icon+"</span></button></div>";
            ...
            sub_fab_btns.find('.sub_fab_btn').on('mousedown', function(e){
                ...
                _log($(this).attr('data-target')+' clicked...');
                
                /*if( $(this).attr('data-target') == ".modalCalzone" ){
                    _log($('select#tamanhoCalzone option').length);
                    if($('select#tamanhoCalzone option').length == 2){
                        //seleciona o tamanho do calzone
                        _log('seleciona tamanho calzone (apenas 1 cadastrado)');
                        $('#tamanhoCalzone').val($('select#tamanhoCalzone option:last').val());
                        var scp = angular.element("#tamanhoCalzone").scope();
                        scp.atualizaOpcoesCalzone(); // aplica o tamanho selecionado
                    }
                }*/
            });
